GET = function (req,res) {
	...
	req.db.all('SELECT sS.*, i.name, i.desc, i.img, f.name AS fName FROM shopStock AS sS INNER JOIN items AS i ON i.itemID = sS.itemID INNER JOIN itemFunctions AS f ON f.functionID = i.functionID ORDER BY sS.itemID ASC', function (err,shopItems) {
		if(err) {
			console.log(err);
