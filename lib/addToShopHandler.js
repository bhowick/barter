	...
	if(!msg || !validator.isNumeric(msg)) {
		...
		switch (msg) {
			...
			case "5":
				...
				break;
			case "6":
				msgText = "That item is already in the shop.";
				break;
			default: 
...
POST = function (req,res) {
	...
	if(!itemID || itemID < 1) {
		...
		//BUT FIRST... we check to see if our item's already in the shop.
		req.db.get('SELECT itemID FROM shopStock WHERE itemID = ?', itemID, function (err,item) {
			if(err) {
				console.log(err);
				res.send(err);
			}
			else if (!item) { //Hmm...
				console.log('Looks like it\'s okay to proceed, as this item isn\'t here yet.');
