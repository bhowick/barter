...
POST = function (req,res) {
	...
	var userCheck = validator.isAlphanumeric(username);
		...
		req.db.get('SELECT userID FROM users WHERE name = ?', username, function(err,row){
			...
			//Let's see...
				...
				if(item){
					...
					var isAdmin = false; //Haha, yeah, no, you're not becoming an admin.
					var rankID = 2; //Sets the rank ID to the default (2)
					...
					var saltBytes = crypto.randomBytes(32); //Generates random bytes.
					...
					var hashedKey = crypto.pbkdf2Sync(password, salt, iter, hashLength, hashType); //The actual hashing... thing. Makes raw bytes.
					...
					var toDB = [username, hashedPassword, rankID, balance, salt, timeCreated, timeCreated, isAdmin];
					req.db.run('INSERT INTO users (name, pass, rankID, balance, salt, timeCreated, lastLogin, isAdmin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', toDB, function (err) {
						if(err) {
