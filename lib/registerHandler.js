		...
		req.db.get('SELECT userID FROM users WHERE name = ?', username, function(err,row){
			...
			//Let's see...
				...
				if(item){
					...
					var toDB = [username, hashedPassword, rankID, balance, salt, timeCreated, timeCreated];
					req.db.run('INSERT INTO users (name, pass, rankID, balance, salt, timeCreated, lastLogin) VALUES (?, ?, ?, ?, ?, ?, ?)', toDB, function (err) {
						if(err) {
